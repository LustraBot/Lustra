export default {
  name: "lureload",
  description: "Owner-only: dynamically reload a command or handler",
  options: [
    {
      name: "target",
  ...
  execute: async (interaction) => {
    ...
    if (!target || !name) {
      return interaction.createMessage({ content: "Usage: /lureload target:<command|handler> name:<name>", flags: 64 });
    }
    ...
    try {
      if (target === "handler") {
        ...
        await interaction.createMessage({ content: `Reloaded handler: \`${name}\``, flags: 64 });
      } else {
        ...
        await interaction.createMessage({ content: `Reloaded command: \`${name}\``, flags: 64 });
      }
    } catch (error) {
      await interaction.createMessage({ content: `Reload failed: \`${error?.message || error}\``, flags: 64 });
    }
  }
};
